...
class SessionManager implements ISessionManager {
  ...
  Future<LatLng?> getLatLng() async {
    try {
      final location = await _secureStorage.read(key: SuperKeys.locationKey);
      if (location != null) {
        final latLng = LatLng.fromJson(jsonDecode(location));

        return Future.value(latLng);
      }
      return null;
    } on Exception {
      await _secureStorage.delete(key: SuperKeys.locationKey);
      rethrow;
    }
  }

  @override
  Future<void> saveLocation({required double lat, required double lng}) async {
    try {
      await _secureStorage.write(
          key: SuperKeys.locationKey, value: jsonEncode(LatLng(lat, lng)));
    } on Exception {
      rethrow;
    }
  }
